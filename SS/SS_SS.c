...
void ss_paste_file_or_directory(int client_sock, SS_Request *request)
{
    ...
    if (packet.info[strlen(packet.info) - 1] == '/')
    {
        ...
        ss_paste_directory(client_sock, source_sock, packet.info, request->Thread_id);
    }
    else
    {
        // Source is a file
...
void ss_copy_file_or_directory(int client_sock, SS_Request *request)
{
    ...
    {
        perror("Error getting stat of source path");
    ...
    if (S_ISDIR(src_stat.st_mode))
    {
        ...
        ss_copy_directory(client_sock, path, -1);
    }
    else
    {
        // SS_Request name_packet = {.error_code = OK, .info = "", .type = REQUEST_PASTE};
...
int ss_copy_directory(int client_sock, char *src, int start)
{
    SS_Request packet;
    memset(&packet, 0, sizeof(packet));
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        if (S_ISDIR(entry_stat.st_mode))
        {
            ...
            if (ss_copy_directory(client_sock, src_path, start) < 0)
            {
