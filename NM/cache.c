...
static char* safe_strdup(const char* str) {
    ...
    if (len >= MAX_PATH_LENGTH) return NULL;  // Path too long
...
static CacheNode* evict_lru(LRUCache* cache) {
    ...
    if (lru == cache->head) return NULL;  // Cache is empty
...
void get_cache_stats(LRUCache* cache, int* size, int* capacity, float* collision_rate) {
    ...
    if (collision_rate) {
        *collision_rate = cache->size > 0 ? 
            (float)cache->collision_count / cache->size : 0.0f;
    }
    pthread_mutex_unlock(&cache->lock);
}
