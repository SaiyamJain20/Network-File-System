...
void *wait_for_second_ack(void *arg)
{
    ...
    if (recv(sock, &secondAck, sizeof(secondAck), 0) < 0)
    {
...
void c_write_to_path(char *path, int sync)
{
    ...
    if (recv(ns_sock, &ack1, sizeof(ack1), 0) <= 0)
    {
        printf("ERROR IN READING PACKET\n");
        close(ns_sock);
        return;
    ...
    int recv_status = recv(ns_sock, &request, sizeof(request), 0);
    if (recv_status <= 0)
    {
        printf("RECV ERROR\n");
        close(ns_sock);
...
void *audio_stream_handler(void *arg)
{
    ...
    while (1)
    {
        ...
        {
            ssize_t r = recv(sockfd, ((char *)&p) + total_received, sizeof(Request) - total_received, 0);
            if (r <= 0)
            {
                if (r == 0)
                {
